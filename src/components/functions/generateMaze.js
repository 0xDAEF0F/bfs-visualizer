...
export default function generateMaze(rows, cols) {
  ...
  return [orderCarved, carved.map((row) => row.map((value) => !value))];
}
...
function noAdjVisNeigh(
  unvisitedNeighbors,
  allNodesVisitedStateArray,
  maxRows,
  maxCols,
  myCurrCoord
) {
  ...
  unvisitedNeighbors.forEach((pairOfCoordinates) => {
    ...
    let newNeighbors = neighbors.filter(
      (neigh) => _.isEqual(neigh, myCurrCoord) === false
    );
    // check for a visited neighbor
    let areAnyVisited = anyVisitedNeighbors(
      newNeighbors,
      allNodesVisitedStateArray
    );
